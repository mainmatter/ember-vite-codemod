...
describe('Test on all Ember versions', function () {
  for (let [version, packages] of testVersions) {
    it(`should work for ember version ${version}`, async function ({ expect }) {
      ...
      // TODO uncomment this when it's actually working
      // const updateScriptPath = join(__dirname, '../index.js');
      // await execaNode({cwd})`${updateScriptPath}`;

      // // TODO move this to just be pnpm install assuming that the script would add vite
      // await execa({cwd})`pnpm i vite`;

      // await execa({cwd})`./node_modules/.bin/vite build`;
      // let result = await execa({cwd})`${cliPath} test --path dist`;
