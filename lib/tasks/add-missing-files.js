...
const files = [['vite.config.mjs', 'files/vite.config.mjs']];

const jsFiles = [
  ['babel.config.cjs', 'files/_js_babel.config.cjs'],
...
async function writeIfMissing([destination, source], projectType) {
  try {
    ...
    if (projectType === 'js' && source.endsWith('.ts')) {
...
export default async function addMissingFiles(options = { ts: false }) {
  ...
  for (let file of filesToAdd) {
    await writeIfMissing(file, projectType);
  }
}
