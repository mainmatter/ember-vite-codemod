...
describe('ensureV2Addons() function', () => {
  ...
  describe('when there is a dependency that was not yet upgraded to v2', () => {
    let inquirerPrompt;
    ...
    it('notifies and stops in non interactive mode', async () => {
      await expect(() =>
        ensureV2Addons({ nonInteractive: true }),
    ...
    it('notifies, prompts and stops when the user choses to stop', async () => {
      inquirerPrompt = vi.spyOn(inquirer, 'prompt').mockImplementation(() => {
        return { continueExec: 'Stop the codemod' };
      });
      await expect(() => ensureV2Addons()).rejects.toThrowError(
        'The codemod was stopped.',
      );
    });

    it('notifies, prompts and continues when the user choses to continue', async () => {
      inquirerPrompt = vi.spyOn(inquirer, 'prompt').mockImplementation(() => {
        return { continueExec: 'Continue the move to Vite' };
      });
      await ensureV2Addons();
    });
  });
