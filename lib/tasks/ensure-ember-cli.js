...
export default async function ensureEmberCli() {
  ...
  const emberSource = packageJSON['devDependencies']?.['ember-source'] || packageJSON['dependencies']?.['ember-source'];
  if (semver.lt(
  ...
  const emberCli = packageJSON['devDependencies']?.['ember-cli'] || packageJSON['dependencies']?.['ember-cli'];
  if (semver.lt(
