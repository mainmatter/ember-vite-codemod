...
import { findYarnWorkspaceRoot } from '../utils/find-yarn-workspace.js';
...
async function getV1Addons({ errorTrace = false, yarnWorkspace = false } = {}) {
  ...
  for (const [depName, version] of Object.entries(deps)) {
    let pkg;
    let pkgInfo;
    let yarnWorkspaceRoot = '';
    try {
      if(yarnWorkspace) {
        yarnWorkspaceRoot = findYarnWorkspaceRoot();
      }

      pkg = JSON.parse(
        await readFile(join( yarnWorkspaceRoot, 'node_modules', depName, 'package.json'), 'utf8'),
      );
      if (pkg) {
