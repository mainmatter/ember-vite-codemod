...
import transformTsConfig from '../transforms/tsconfig.js';
...
export default async function modifyFiles(options) {
  ...
  if (emberVersion) {
    emberVersion = coerce(emberVersion);
  }
  ...
  if (options.ts) {
    if (existsSync('types/index.d.ts')) {
      ...
      await transformWithReplace('tsconfig.json', transformTsConfig);
    }
  }
  ...
  await transformWithAst(resolveWithExt('app/app'), transformApp, {
    ...options,
    emberVersion,
  });
  await transformWithAst(
