...
function updateVersion(deps, depToAdd, minimumVersion) {
  ...
  if (deps[depToAdd]) {
    ...
    // it should always be possible to run this codemod multiple times and it be
    // idempotent (i.e. only actually update files that need updating). We check
    // that version is not undefined here because calling `semver.coerce()` on a
    // undefined value throws an error and that can start happening when we have
    // removed packages from the package.json
    if (
      version &&
      semver.lt(
