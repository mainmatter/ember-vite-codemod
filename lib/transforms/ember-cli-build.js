      ...
      this.traverse(path, {
        visitReturnStatement(path) {
          ...
          if (
            ...
            optionsNode = path.value.argument.arguments[2];
          }
...
function removeWebpackImport(ast) {
  visit(ast, {
    visitVariableDeclaration(path) {
      if (path.value.declarations[0].init.type === 'CallExpression' &&
        path.value.declarations[0].init.arguments.length === 1 &&
        path.value.declarations[0].init.arguments[0].type === 'StringLiteral' &&
        path.value.declarations[0].init.arguments[0].value === '@embroider/webpack'
      ) {
        // Expect parent.path to be a "body" array
        let body = path.parentPath;
        if (body.value.length) {
          let propWebpackIndex = body.value.findIndex((node) => node === path.value);
