...
export default async function transformTestHelper(ast) {
  ...
  if (emberQunitImport) {
    ...
    if (startSpecifierIndex > -1) {
      emberQunitImport.specifiers[startSpecifierIndex] = b.importSpecifier(
        b.identifier('start'),
        b.identifier('qunitStart'),
      );
    }
  } else {
    console.log(
  ...
  if (emberExamImport) {
    emberExamImport.source.value = 'ember-exam/addon-test-support';
    const startSpecifierIndex = emberExamImport.specifiers.findIndex(
      (specifier) => {
        return (
          specifier.type === 'ImportSpecifier' &&
          specifier.imported.type === 'Identifier' &&
    ...
    if (startSpecifierIndex > -1) {
      emberExamImport.specifiers[startSpecifierIndex] = b.importSpecifier(
        b.identifier('start'),
    ...
    visitCallExpression(path) {
      if (path.value.callee.type === 'Identifier') {
        ...
        if (name === 'start') {
          ...
          return false;
        } else if (name === 'setupEmberOnerrorValidation') {
          hasEmberOnerrorValidationCall = true;
        }
      }
      this.traverse(path);
    },
  ...
  const setupEmberOnerrorValidationImport = emberQunitImport.specifiers.find(
    (specifier) => {
      return (
        specifier.type === 'ImportSpecifier' &&
        specifier.imported.type === 'Identifier' &&
        specifier.imported.name === 'setupEmberOnerrorValidation'
      );
    },
  );
  // Add import
  if (!setupEmberOnerrorValidationImport) {
    emberQunitImport.specifiers.push(
      b.importSpecifier(b.identifier('setupEmberOnerrorValidation')),
    );
  }
  // Add call
  if (!hasEmberOnerrorValidationCall) {
    let qunitStartIndex = nodesToWrap.findIndex(
      (node) => node.expression?.identifier?.name === 'qunitStart',
    );
    let expression = b.expressionStatement(
      b.callExpression(b.identifier('setupEmberOnerrorValidation'), []),
    );
